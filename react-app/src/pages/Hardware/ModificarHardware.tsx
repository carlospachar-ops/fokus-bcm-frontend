...
export default function EditarHardware() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [prov, prop, hw, itemRes] = await Promise.all([
          ...
          fetch(`${baseUrl}/cfg/hardware/${id}`).then(r => r.json()), 
        ]);

        setProveedores(prov.data ?? []);
        setPropietarios(prop.data ?? []);
        
        const catalogoHw = hw.data ?? [];
        setHardwares(catalogoHw.filter((h: Option) => h.id !== Number(id)));
        ...
        if (item) {
          setForm({
            ...
            id_propietario: item.id_propietario ? String(item.id_propietario) : "",
            serial: item.serial || "",
            rto_horas: item.rto_horas ? String(item.rto_horas) : "",
            impacto_negocio: item.impacto_negocio || "",
            dependencias_ids: item.dependencias_ids || [],
          });
        }
      } catch (error) {
        // Como es solo interfaz, ignoramos el error silenciosamente
        console.warn("Backend no conectado - cargando formulario vacío para demostración visual", error);
      } finally {
        setFetching(false); // Quitamos el loading para mostrar la vista
      }
    };
  ...
  const toggleDependencia = (idHw: number) => {
    setForm(prev => ({
      ...prev,
      dependencias_ids: prev.dependencias_ids.includes(idHw)
        ? prev.dependencias_ids.filter(d => d !== idHw)
        : [...prev.dependencias_ids, idHw],
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await fetch(`${baseUrl}/cfg/hardware/${id}`, {
        method: "PUT",
      ...
      // Como no hay backend, forzamos la navegación para simular que todo salió bien
      console.warn("Backend no conectado - simulando guardado exitoso", error);
      navigate("/configuracion");
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="container py-4 nuevoHardware" style={{ maxWidth: 980 }}>
      <h3 className="mb-4 nuevoHardware-title">Editar hardware</h3>
